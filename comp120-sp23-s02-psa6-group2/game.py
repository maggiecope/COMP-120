...
class Game:
    ...
    def __init__(self, max_depth: int,
        ...
        """Initialize this game, as described in the PSA description.
        ...
        #The number of players will be the sum of the human, random, and smart players
        num_players = num_human + random_players + len(smart_players)

        #generate a random goal type, for all players to share
        goal = []
        n = random.random_init(BlobGoal,PerimeterGoal)
        for i in range(num_players):
            #Assign each of the players a random target color 
            goal.append(n(COLOR_LIST[random.random_init(0,len(COLOR_LIST))]))


        #generate a random board with the given maximum depth
        self.board = random.random_init(0,max_depth)
        
        #Generate the right number of human players, random players, and smart players 
        #(with the given difficulty levels), in that order
        #Give the players consecutive player numbers, starting at 0. 
        for i in range(num_players):
            if i < num_human:
                self.players.append(HumanPlayer(self.renderer, i, goal[i]))
            elif i < random_players:
                self.players.append(RandomPlayer(self.renderer, i, goal[i]))
            else: 
                self.players.append(SmartPlayer(self.renderer, i, goal[i]))
            #draw the board 
            self.renderer.draw.(self.board, i)
       
        for player in self.players:
            #display the the players goal to them 
